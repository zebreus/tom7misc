...
static inline auto FindWithDefaultFirst(
    const C &cont, const K &key, V v) -> V {
  auto cit = cont.find(key);
  if (cit == cont.end()) return v;
  return cit->second.first;
}

static inline constexpr uint32 HalfAlpha(uint32_t c) {
...
ImageRGBA ErrorHistory::MakeImage(
    int width, int height,
    const std::map<int, std::pair<uint32_t, string>> &column_info,
    int model_idx) const {
  ...
  for (const auto &record : records) {
    if (record.model_index == model_idx &&
        column_info.find(record.column_index) != column_info.end()) {
      collated_records[record.round_number].push_back(record);
    }
  }
  ...
  auto ValuesForRound = [&](double prev_round, double round) ->
    ...
    // For each column:
    for (const auto &[col, info] : column_info) {
  ...
  for (const auto &[col, values] : ValuesForRound(0.0, max_round)) {
    const uint32 color =
      FindWithDefaultFirst(column_info, col, 0xFF0000FF);
  ...
  {
    int yy = 0;
    for (const auto &[id, info] : column_info) {
      const auto &[color, name] = info;
      image.BlendText32(width - name.size() * 9 - 1, yy,
                        color, name);
      yy += 10;
    }
  }
  ...
  const string maxr = StringPrintf("%lld", max_round);
