...
// Older, slower versino. This can probably be deleted once it's
// benchmarked against the new one.
...
struct NWaysGPUv1 {
  ...
  NWaysGPUv1(CL *cl, int height) : cl(cl), height(height) {
    std::string defines = StringPrintf("#define MAX_WAYS %d\n",
                                       MAX_WAYS);
    std::string kernel_src = defines + Util::ReadFile("sos-v1.cl");
