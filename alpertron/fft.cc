...
void fftMultiplication(const limb *factor1, const limb *factor2, limb *result,
  ...
  static struct sComplex *product = (struct sComplex*)malloc(sizeof (struct sComplex) * MAX_FFT_LEN);
  static struct sComplex *tempFFT = (struct sComplex*)malloc(sizeof (struct sComplex) * MAX_FFT_LEN);
  static struct sComplex *MontgomeryMultNTransf = (struct sComplex*)malloc(sizeof (struct sComplex) * MAX_FFT_LEN);
  static struct sComplex *TestNbrTransf = (struct sComplex*)malloc(sizeof (struct sComplex) * MAX_FFT_LEN);
