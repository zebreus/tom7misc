...
/* SPI bit order. BCM2835 SPI0 only supports MSBFIRST, so we instead
 have a software based bit reversal, based on a contribution by
 Damiano Benedetti */
static uint8_t bcm2835_spi_bit_order =
  BCM2835_SPI_BIT_ORDER_MSBFIRST;
static constexpr uint8_t bcm2835_byte_reverse_table[] = {
...
uint8_t bcm2835_spi_transfer(uint8_t value) {
  ...
  BUG ALERT: what happens if we get interrupted in this section,
  and someone else accesses a different peripheral?
  Clear TX and RX fifos
  */
  ...
  /* Read any byte that was sent back by the slave while we sere
     sending to it */
...
// The divisor must be a power of 2 (this might be a copy paste error from
// spi docs? the enums are not powers of two -tom7). Odd numbers
// rounded down.
*/
