...
typedef uint32_t v32x4 __attribute__ ((vector_size (128)));
...
void ImageRGBA::BlendPixel(int x, int y,
  ...
  const word old_r = rgba[i + 0];
  const word old_g = rgba[i + 1];
  const word old_b = rgba[i + 2];
  ...
  // Note that the result cannot be > 0xFF.
