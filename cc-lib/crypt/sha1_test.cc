...
static void TestHMAC() {
...

  {
    ...
    CHECK(key.size() == 25);
    std::vector<uint8_t> message(50, 0xcd);
    std::vector<uint8_t> expected = {
...

  {
    ...
    std::vector<uint8_t> message = StringVec("Test With Truncation");
...

  {
    ...
    CHECK(message.size() == 73);

    // Note: There appears to be a copy-paste problem in the RFC here.
    // There are multiple digests listed, but some are from the
    // "test with truncation" input above.
    std::vector<uint8_t> expected = {
