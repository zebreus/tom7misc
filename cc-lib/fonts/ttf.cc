...
string TTF::Font::ToSFD() const {
  CHECK(!family_name.empty()) << "Need a name for the font.";
  ...
  // This is "Name ID 6". Needs to be sanitized for postscript.
  string name_postscript;
  ...
  std::string ribbi_string;

  // Similarly: The "Full Name" is just the family name plus the
  // subfamily (if not "regular"), of which we only support the RIBBI
  // four.
  std::string full_name = family_name;
  int mac_style = 0;
  int italic_angle = 0;
  switch (ribbi) {
  case RIBBI::REGULAR:
    ribbi_string = "Regular";
    break;
  case RIBBI::BOLD:
    ribbi_string = "Bold";
    name_postscript.append("-Bold");
    full_name.append(" Bold");
    mac_style = 1;
    break;
  case RIBBI::ITALIC:
    ribbi_string = "Italic";
    name_postscript.append("-Italic");
    full_name.append(" Italic");
    mac_style = 2;
    italic_angle = -12;
    break;
  case RIBBI::BOLD_ITALIC:
    ribbi_string = "Bold Italic";
    name_postscript.append("-BoldItalic");
    full_name.append(" Bold Italic");
    mac_style = 1 | 2;
    italic_angle = -12;
    break;
  default:
  ...
  CHECK(name_postscript.length() <= 63)
    << "PostScript name exceeds 63 characters: " << name_postscript;
  ...
  // Not clear if this will have any effect for TTF, but FontForge might use
  // it to derive other values not specified.
  uint8_t pfm_family = (uint8_t)(FF_DONTCARE << 8) | (uint8_t)DEFAULT_PITCH;
  if (monospace) {
    pfm_family = (uint8_t)(FF_DONTCARE << 8) | (uint8_t)FIXED_PITCH;
  }

  int panose_weight = (ribbi == RIBBI::BOLD ||
                       ribbi == RIBBI::BOLD_ITALIC) ? 8 : 5;
  int panose_proportion = monospace ? 9 : 3;
  // Latin text, sans-serif, weight, proportion, etc.
  std::string panose = std::format("Panose: 2 11 {} {} 0 0 0 0 0 0\n",
                                   panose_weight, panose_proportion);
...
ribbi_string, mac_style,
pfm_family,
copyright,
italic_angle,
ascent, descent, weight, width,
...
antialias_flag);
  ...
  out.append(gasp_and_prep);
  AppendFormat(&out, "BeginChars: 1114112 {}\n", numchars);
  ...
  auto CharToSFD = [&](int codepoint, const Char &ch, int index) {
      string char_name;
      {
        ...
        if (it == names.end()) {
          if (codepoint <= 0xFFFF) {
            char_name = std::format("U+{:04x}", codepoint);
          } else {
            // This is what FontForge output for U+1F680 (rocket emoji),
            // at least.
            char_name = std::format("u{:X}", codepoint);
          }
        } else {
      ...
      string ret = std::format(
          ...
          "Encoding: {} {} {}\n"
          // Explicit (W)idth, maybe also (M)ark if a combining char.
          "Flags: W{}\n"
          "Width: {}\n"
          "Layercount: 2\n"
          "Fore\n",
      ...
      if (!ch.contours.empty()) {
        ...
        for (const Contour &ucontour : ch.contours) {
          ...
          for (const Path &path : contour.paths) {
            ...
            switch (path.type) {
            case PathType::LINE:
              AppendFormat(&ret, " {} {} l 0\n", x, y);
              break;
            case PathType::BEZIER: {
              int cx = MapX(path.cx);
              int cy = MapY(path.cy);
              // Seems that FontForge wants two control points (this
              // is how it saves quad beziers itself), so duplicate.
              AppendFormat(&ret, " {} {} {} {} {} {} c 0\n",
                           cx, cy, cx, cy, x, y);
              break;
            }
            }
