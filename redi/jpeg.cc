...
static void Quantize(RediImageRGBA *img) {
  // three bit color
...
[[maybe_unused]]
static void BlitNodeChannel(int ch_offset, const RediImageA &channel, int xpos,
                            int ypos, vector<float> *dest) {
        ...
        for (int ch = 0; ch < show_channels; ch++) {
          ...
          for (int layer = 0; layer < NUM_LAYERS + 1; layer++) {
            const vector<float> &values =
                current_stimulations[ex].values[layer];
            int sy = MARGIN + layer * (SIZE + GAP);
            ...
            if (ox >= 0 && oy >= 0 && ox < SIZE && oy < SIZE) {
              ...
              if (layer > 0) {
                ...
                for (int i = 0; i < INDICES_PER_NODE; i++) {
                  ...
                  if (weight > 0) {
                    sdlutil::drawpixel(screen, sx + src_x, sy + src_y, 0xFF,
                                       0xFF, 0);
                  } else {
      ...
      } else if (mode == MODE_EVALUATE) {
        {
          ...
          for (int src = 0; src < NUM_LAYERS; src++) {
            ...
            ParallelComp(
                ...
                [&DrawStimulus, &heldout_corpus, &stims, &fc, &fonts, mousex,
                 mousey, src, offset](int column) {
                  ...
                  if (src == 0) {
                    ...
                    if (mx == column &&
                      ...
                      if (ox >= 0 && oy >= 0 && ox < SIZE && oy < SIZE) {
                        ...
                        BlitChannel(0xFF, 0x0, 0x0, *fonts[0], x_dice, y_dice,
                                    img);
                      }
                    }
...
void TrainThread() {
  ...
  string start_seed = StringPrintf("%d  %lld",
                                   Util::getpid(), (int64_t)time(nullptr));
  Printf("Start seed: [%s]\n", start_seed.c_str());
