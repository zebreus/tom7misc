...
struct Connection {
  ...
  Connection(const std::string &db_file) {
    int rc = sqlite3_open(db_file.c_str(), &db);
    CHECK(rc == SQLITE_OK) << "Couldn't open database from " << db_file << "\n"
                           << ErrString(rc);
  }

  void ExecuteString(const std::string &s) {
    sqlite3_stmt *stmt = nullptr;
    int rc = sqlite3_prepare_v3(db, s.data(), (int)s.size(),
                                // flags
                                0,
                                &stmt,
                                nullptr);
    CHECK(rc == SQLITE_OK) << "Could not prepare " << s;
    ...
    for (;;) {
      ...
      if (src == SQLITE_ROW) {
        ...
        for (int i = 0; i < cols; i++) {
          ...
          case SQLITE_FLOAT:
            printf("%.11g", sqlite3_column_double(stmt, i));
            break;
...
int main(int argc, char **argv) {
  ...
  Connection conn("test.sqlite");

  conn.ExecuteString("create table test ("
                     "id int not null, "
                     "name varchar(255) not null)");

  conn.ExecuteString("insert into test (id, name) "
                     "values (7, \"Tom\")");

  conn.ExecuteString("select * from test");
