...
template <class F> void NonUnityProducts(const vector<int> &factors, F f) {
  // First, the full list...
  f(factors);
  ...
  for (int i = 0; i < factors.size(); i++) {
    for (int j = i + 1; j < factors.size(); j++) {
      ...
      for (int x = 0; x < factors.size(); x++) {
        if (x == i)
          fs.push_back(factors[i] * factors[j]);
        else if (x != j)
          fs.push_back(factors[x]);
      }
...
template <class F>
void SubsetsProductingTo(int target, int nfactors, int *factors,
  ...
  for (int bus = 0; bus < 100; bus++) {
    ...
    for (int wiz = 1; wiz < 100; wiz++) {
      ...
      */
      ...
      auto Consider = [&sols](vector<int> children) {
      ...
      NonUnityProducts(factors, [bus, &Consider](const vector<int> &children) {
        int sum = 0;
        for (int c : children)
          sum += c;
        if (sum > bus)
