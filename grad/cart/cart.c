  ...
  *PPU_MASK = 0x1e;
  ...
  for (;;) {
    ...
    #define CHUNKS_ACROSS (32 / CHUNK_SIZE)
    {
      ...
      if (bank == 0 && sy < 30) {
        unsigned short screen_start =
          0x2000U + (unsigned short)sy * 32 + sx;
        unsigned char *MEM_LINE =
          MEM + bank +
          (unsigned short)chunk_num * CHUNK_SIZE;
