...
struct Stats {
  ...
  void Progress(int64 total) {
    ...
    fprintf(stderr,
            "Ran %lldk/%dM (" AYELLOW("%.1f%%") "; "
            ACYAN("%d") "/s). "
            AGREEN("%lld") " new " ABLUE("%lld") " opt, "
            ARED("%lld") " infeasible (" ARED("%.1f%%")")\n",
  ...
  void Report() {
    std::unique_lock<std::mutex> ml(m);
    printf("Total done: " AYELLOW("%lld") "\n"
           "Infeasible: " ARED("%lld") "\n"
           "Added new: " AGREEN("%lld") "\n"
           "Added smaller: " ABLUE("%lld") "\n"
           "Not choppy: " ACYAN("%lld") "\n"
           "Outside grid: " ACYAN("%lld") "\n"
           "Not new: " ACYAN("%lld") "\n",
...
static void ExpandNegate(DB *db) {
  printf(ANSI_PURPLE "-- Negate --" ANSI_RESET "\n");
...
static void ExpandSquish(DB *db) {
  printf(ANSI_PURPLE "-- Squish --" ANSI_RESET "\n");
  // Copy so that we don't have to deal with iterator
  // invalidation.
  std::vector<const Exp *> all;
  for (const auto &[k, v] : db->fns) all.push_back(v);
  ...
  for (const Exp *e : all) {
    Allocator alloc;
    for (int scale = 2; scale < 16; scale = scale * 2) {
      stats.done++;
      const Exp *scaled =
        Exp::Subst(&alloc,
                   e,
                   alloc.TimesC(var, Exp::GetU16((half)scale)));

      DB::AddResult ar = db->Add(scaled);
      stats.Observe(ar);
    }
  }
...
static void ExpandShift(DB *db) {
  printf(ANSI_PURPLE "-- Shift --" ANSI_RESET "\n");
  // Copy so that we don't have to deal with iterator
  // invalidation.
  std::vector<const Exp *> all;
  for (const auto &[k, v] : db->fns) all.push_back(v);
  ...
  for (int idx = 0; idx < all.size(); idx++) {
    const Exp *e = all[idx];
    Allocator alloc;
    for (int xo = -Choppy::GRID/2; xo <= Choppy::GRID/2; xo++) {
      if (xo != 0) {
        stats.done++;
    ...
    if (idx % 100 == 0) {
      printf("Expanded %d/%d (%.4f/sec)\n", idx, (int)all.size(),
             idx / timer.Seconds());
      stats.Progress(all.size() * Choppy::GRID);
    }
  }
