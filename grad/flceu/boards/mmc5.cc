...
void MMC5::MMC5WRAM(uint32 A, uint32 V) {
  // printf("%02x\n",V);
  V = MMC5WRAMIndex[V & 7];
  if (V != 255) {
...
void MMC5::Mapper5_write(DECLFW_ARGS) {
...
DECLFR_RET MMC5::MMC5_ReadROMRAM(DECLFR_ARGS) {
...
void MMC5::MMC5_WriteROMRAM(DECLFW_ARGS) {
  if (A >= 0x8000)
...
void MMC5::MMC5_ExRAMWr(DECLFW_ARGS) {
  if (fc->ppu->MMC5HackCHRMode != 3) ExRAM[A & 0x3ff] = V;
}

DECLFR_RET MMC5::MMC5_ExRAMRd(DECLFR_ARGS) {
...
void MMC5::MMC5Synco() {
  ...
  {
    const uint32 t = NTFill | (NTFill << 8) | (NTFill << 16) | (NTFill << 24);
    FCEU_dwmemset(MMC5fill, t, 0x3c0);
  }

  {
    const unsigned char moop =
      ATFill | (ATFill << 2) | (ATFill << 4) | (ATFill << 6);
    const uint32 t = moop | (moop << 8) | (moop << 16) | (moop << 24);
    FCEU_dwmemset(MMC5fill + 0x3c0, t, 0x40);
  }
  fc->X->IRQEnd(FCEU_IQEXT);
...
void MMC5::Do5PCMHQ() {
  if (!(MMC5Sound.rawcontrol & 0x40) && MMC5Sound.raw)
    for (uint32 V = MMC5Sound.BC[2]; V < fc->sound->SoundTS(); V++)
      fc->sound->WaveHi[V] += MMC5Sound.raw << 5;
  MMC5Sound.BC[2] = fc->sound->SoundTS();
}

void MMC5::Mapper5_SW(DECLFW_ARGS) {
  A &= 0x1F;
...
void MMC5::MMC5RunSoundHQ() {
  Do5SQHQ(0);
  Do5SQHQ(1);
  Do5PCMHQ();
}

void MMC5::MMC5HiSync(int32 ts) {
  for (int x = 0; x < 3; x++) MMC5Sound.BC[x] = ts;
}

void MMC5::MMC5RunSound(int count) {
  Do5SQ(0);
  Do5SQ(1);
  Do5PCM();
...
void MMC5::Power() {
  // Initialize, a la cart.cc. These addresses are *before* nothing,
  // because offsets are "pre-subtracted" (we index into
  // VPage[addr >> 10][addr], rather than VPage[addr >> 10][addr & 1023]).
  for (int x = 0; x < 8; x++)
...
MMC5::MMC5(FC *fc, CartInfo *info, int wsize, int battery) :
    CartInterface(fc) {
  if (wsize) {
  ...
  // XXX perhaps these variables should be moved to here from PPU? -tom7
