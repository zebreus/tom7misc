...
__kernel void ForwardChunkSparse(
  ...
  // Start with bias.
  float potential = ToHalf(bias[node_idx]);
  ...
  for (int i = 0; i < INDICES_PER_NODE; i++) {
    ...
    const float w = ToHalf(my_weights[i]);
    // PERF: We can maintain a representation invariant that the outputs
    // are always rounded to the nearest half, yeah?
    const float v = ToHalf(previous_layer_outputs[src_start + in_idx]);
    // potential += w * v;
...
__kernel void ForwardChunkDense(
  ...
  for (int i = 0; i < INDICES_PER_NODE; i++) {
    const float w = ToHalf(my_weights[i]);
    const float v =
      ToHalf(previous_layer_outputs[src_start + SPAN_START + i]);
    // potential += w * v;
    // potential = fma(w, v, potential);
    potential = ToHalf(ToHalf(w * v) + potential);
  }
...
__kernel void ForwardChunkConvolutional(
  ...
  float potential = ToHalf(bias[feature_number]);
  ...
  for (int y = 0; y < PATTERN_HEIGHT; y++) {
    ...
    for (int x = 0; x < PATTERN_WIDTH; x++) {
      ...
      const float w = ToHalf(feature_weights[i]);
      const float v =
        ToHalf(previous_layer_outputs[src_start + in_idx]);
