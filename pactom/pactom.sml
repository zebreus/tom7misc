...
struct
  ...
  fun randf () =
    real (Word32.toInt (Word32.andb(0wx7FFFFFFE,
                                    MersenneTwister.rand32 seed))) /
    real 0x7FFFFFFE
  ...
  fun fromkmlfiles fs =
    let
      ...
      fun process (Elem(("coordinates", nil), [Text coordtext])) =
        let
          val coords = String.tokens (StringUtil.charspec " \t\r\n") coordtext
          val coords =
            map (fn t =>
      ...
      | process (t as Elem(("GroundOverlay", _), _)) =
        let
          ...
          fun findoneopt tag =
            ...
            | SOME _ =>
                raise PacTom ("GroundOverlay specified more than one " ^ tag)
  ...
  fun neighborhoodsfromkml f =
              ...
              let
                ...
                fun parsecoords t =
                  (case map Real.fromString (String.fields (Util.is #",") t) of
                     [SOME lon, SOME lat, SOME _] =>
                       LatLon.fromdegs {lon = lon, lat = lat}
                   | _ => raise PacTom ("non-numeric lat/lon/elev: " ^ t))
                val coords = map parsecoords coords
              in
                case !name of
    ...
    in
      TextIO.output (TextIO.stdErr,
                     " ... Done in " ^
                     LargeInt.toString (Time.toSeconds
                                        (Time.-(Time.now(), start))) ^
                     " sec\n");
  ...
  fun projecthoods projection (hoods : neighborhoods) :
    ...
    let
      ...
      val hoods =
        Vector.map (fn (name, poly) =>
                    (name, Vector.map projection poly)) hoods
...
*)
    ...
    in
      Vector.app (fn (_, poly) =>
                  Vector.app (Bounds.boundpoint bounds) poly) hoods;
    ...
    let
      ...
      fun oneloop error a b nil = [a, b]
        | oneloop error a b (c :: rest) =
          ...
          in
            (* XXX this can chop off points from a perfectly straight
               trip up a dead end with a turn-around, because that
               turn-around has zero area. Should be some kind of max
               angle or extremity check for B too. *)
...
*)
  ...
  fun spanning_graph (pactom : pactom) (rootnear : LatLon.pos) :
    ...
    let
      ...
      val { graph = shortest_g, promote = shortest_p } =
        G.shortestpaths root_node
      (* XXX with this graph, could make a histogram of road-distance from
         home *)
