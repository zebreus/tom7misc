...
struct
  ...
  val () =
    msg ("There are " ^ Int.toString (Vector.length (PacTom.paths pt)) ^
         " paths\n" ^
  ...
  val () =
    let
      val gnomonic : LatLon.projection = LatLon.gnomonic PacTom.home
      fun projection p =
        let val (x, y) = gnomonic p
        in
      ...
      val { borders, bounds = hoodbounds } =
        PacTom.projecthoods projection hoods
      ...
      fun printhoodfill (name, poly) =
        let
          val pts = Vector.foldr op:: nil poly
          val pts = pts @ [hd pts]
        in
          print ("<polygon fill=\"#" ^
