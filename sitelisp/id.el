...
(defun insert-id ()
  ...
  (progn
    ...
    (insert (if (null buffer-file-name) "unknown" (car (reverse (split-string buffer-file-name "/")))))
    (insert ",v 1.00")
...
(defun update-ids ()
  ...
  (save-excursion
	...
	(while (search-forward-regexp 
		"$Id: [A-Za-z0-9_.-]+,v \\([0-9]+\\.?[0-9]*\\)" nil t)
	  (let* ((start (match-beginning 1))
		 (old-vers (split-string (match-string-no-properties 1) "\\."))
		 (major-vers (if (null old-vers) 1 (string-to-number (car old-vers))))
		 (minor-vers (if (null (cdr old-vers)) 0 (string-to-number (cadr old-vers)))))
;;	    (message old-vers)
	    (search-forward " Exp $")
...
(defvar date-format "%-e %b %Y" "*Customizable format for insert-datestamp (\\[insert-datestamp]).")
(defvar time-format "%H:%M:%S" "*Customizable format for insert-timestamp. (\\[insert-timestamp]).")

(defun insert-timestamp ()
  "Inserts the current time into the buffer using the value of time-format."
  (interactive "*")
  (insert (format-time-string time-format (current-time))))

(defun insert-datestamp ()
  "Inserts the current date into the buffer using the value of date-format."
  (interactive "*")
