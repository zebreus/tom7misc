...
;; todo: datatype x = datatype y

;;; sml-mode.el. Major mode for editing (Standard) ML. Version T7.1

;; Copyright (C) 1989, Lars Bo Nielsen; 1994,1997, Matthew J. Morley
;; 2003, Tom Murphy

;; $Revision: 1.5 $
;; $Date: 2006/06/19 20:10:16 $
...
;; to Tom it's much improved!

;; Tom has taken over this version, and plans to destroy compatibility
;; with other sml-modes so that he can make it work the way he wants.
...
(defvar sml-structure-indent 0          ; Not currently an option.
...
(defun install-sml-keybindings (map)
  ...
  (define-key map "\M-q"     'sml-fill-comment)
  (define-key map "\;"       'sml-electric-semi)
  (define-key map "\M-*"     'sml-insert-comment)
...
;; tom 7 removed 'structure' 'functor' and 'signature' as per
;; his personal pref.
;; "structure\\b\\|signature\\b\\|functor\\b"
;; I also removed 'abstraction' since this is not a SML 97 keyword.

(defconst sml-indent-starters-reg
  "abstype\\b\\|and\\b\\|case\\b\\|datatype\\b\
\\|else\\b\\|fun\\b\\|sharing\\b\
\\|in\\b\\|infix\\b\\|infixr\\b\\|let\\b\\|if\\b\\|then\\b\\|local\\b\
\\|nonfix\\b\\|of\\b\\|open\\b\\|raise\\b\\|sig\\b\
\\|struct\\b\\|\\btype\\b\\|val\\b\
\\|while\\b\\|with\\b\\|withtype\\b"
  "The indentation starters. The next line will be indented.")

;; ditto abstraction
(defconst sml-starters-reg
  "\\babstype\\b\\|\\bdatatype\\b\
    ...
    (let ((levelpar 0)                  ; Level of "()"
      ...
      (catch 'loop
        ...
        (if (save-excursion
              (forward-char 1)
              (looking-at sml-indent-starters-reg))
            (+ ilevel (current-column) sml-indent-level)
...
;; Field part of path (Structure.Structure.field)
...
;;;;;;;;;;;;;;;;;;;;;; SML TOPLEVEL COLORING ;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;;;;;;;;;;;;;;;;;;;;;; XXX does not work. Please fix. ;;;;;;;;;;;;;;;;;;;
(require 'comint)

(defvar sml-toplevel-prompt-regexp "^[\-=] *"
  "*Regexp used to recognise prompts in the inferior ML process.")

(defconst sml-toplevel-error-regexp
  (concat
   "^[-= ]*\\(.+\\):"                     ;file name
...
(defvar sml-toplevel-mode-map nil)

;; buffer-local

(defun sml-toplevel-mode ()
  (interactive)
  (kill-all-local-variables)
  (shell-mode)
  (setq comint-prompt-regexp sml-toplevel-prompt-regexp)
  (sml-mode-variables) ;; ?

  (setq major-mode 'sml-toplevel-mode)
  (setq mode-name "SML Toplevel")
  (setq mode-line-process '(": %s"))
  (use-local-map sml-toplevel-mode-map)
  (turn-on-font-lock)
...
;;;;;; end crazy thing that doesn't work
