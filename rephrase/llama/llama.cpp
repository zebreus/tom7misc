...
static std::vector<llama_vocab::id> llama_tokenize_internal(
    const llama_vocab & vocab, std::string raw_text, bool add_special, bool parse_special = false, bool add_leading_space = true);
...
static std::vector<llama_vocab::id> llama_tokenize_internal(const llama_vocab & vocab, std::string raw_text, bool add_special, bool parse_special, bool add_leading_space) {
        ...
        case LLAMA_VOCAB_TYPE_SPM:
            {
                ...
                for (const auto & fragment : fragment_buffer) {
                    if (fragment.type == FRAGMENT_BUFFER_VARIANT_TYPE_RAW_TEXT) {
                        ...
                        if (add_leading_space && &fragment == &fragment_buffer.front()) {
...
int32_t llama_tokenize_ex(
    ...
    auto res = llama_tokenize_internal(model->vocab, std::string(text, text_len), add_special, parse_special, add_leading_space);
