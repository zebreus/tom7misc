...
const Font *TalkDocument::GetDefaultFont() {
  ...
  return GetFontByName("FixederSys2x");
}
...
void TalkDocument::GenerateOutput(std::string_view filename_base,
  ...
  std::string talk_filename = (std::string)filename_base + ".talk";
  std::string talk =
    "# Generated by BoVeX\n\n";
  ...
  for (const auto &[page_idx, doc] : pages) {
    Talk::Page *pdf_page = pdf->AppendNewPage();
    pageptrs.emplace_back(std::make_unique<TalkPage>(
    ...
    // TalkFont(pdf->GetBuiltInFont(Talk::BuiltInFont::TIMES_ROMAN));
