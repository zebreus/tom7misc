...
void TalkDocument::GenerateOutput(std::string_view filename_base,
  ...
  std::string talk_dir = (std::string)filename_base + "_slides";
  ...
  for (const auto &page : pageptrs) {
    const std::string slidefile =
      Util::dirplus(talk_dir, StringPrintf("slide%d.png", num_pages));
    StringAppendF(&talk,
                  "slide\n"
                  "  %s\n",
                  slidefile.c_str());
    page->image->Save(slidefile);
  }
