...
struct ReducerImpl : public Reducer {
  const CellLibrary &library;
  ArcFour rc;

  ReducerImpl(const CellLibrary &library) : library(library), rc("reducer") {}
  ...
  Layout ReduceWhile(Layout layout, int max_consecutive_failures,
                     std::function<bool(const Layout &layout)> pred) override {
...
Reducer::Reducer() {}
Reducer::~Reducer() {}
std::unique_ptr<Reducer> Reducer::Create(const CellLibrary &library) {
  return std::make_unique<ReducerImpl>(library);
}
