  ...
  for (int64 rounds = 0LL; true; rounds++) {
    ...
    if (rounds % 12345678 == 0) {
      printf(
          ...
          (double)rounds / 1e6, left_citations / 1e9, right_citations / 1e9,
          left_articles / 1e9, right_articles / 1e9, left_won / 1e9,
          right_won / 1e9, ((double)left_citations / right_citations),
          ((double)left_articles / right_articles), ((double)left_won / rounds),
          ((double)right_won / rounds));
    }
  }
