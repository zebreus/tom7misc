...
int main(int argc, char **argv) {
  ...
  CHECK_EQ(argc, 5) << "bestworst.exe infile.txt output-best.txt "
                       "output-worst.txt output-most.txt\n";
  ...
  RE2 line_re{"([^\t]*)\t([\\d]+)\t([\\d]+)"};
  LocalForEachLine(infile, [&cite_stats, &line_re, &articles_kept,
                            &citations_kept](string line) {
    string author;
  ...
  printf("Got stats for %lld keys.\n"
         ...
         (int64)cite_stats.size(), articles_kept, citations_kept, avg);
  ...
  cite_stats.clear();
  printf("Total keys with %lld+ articles and %lld+ citations: %lld\nSorting...",
         MIN_ARTICLES, MIN_CITATIONS, rows.size());
  ...
  std::sort(
      rows.begin(), rows.end(),
      [](const std::pair<string, Stats> &a, const std::pair<string, Stats> &b) {
      ...
      });
  {
    ...
    for (int i = 0; i < 5000 && i < rows.size(); i++) {
      ...
      fprintf(out, "%d.  %s\t%lld\t%lld\t%.6f\t%.6f\n", i, row.first.c_str(),
              row.second.articles, row.second.citations,
