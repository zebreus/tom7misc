...
struct BackwardLayerCL {
  ...
  struct Context {
    ...
    void Backward(TrainingRoundGPU *train) {
      ...
      // This is the source layer, but num_nodes is offset by one since it
      // includes the size of the input layer as element 0.
...
static void RandomizeNetwork(ArcFour *rc, Network *net) {
  ...
  ParallelComp(
      net->num_layers,
      [rcs, &RandomizeFloats, &net](int layer) {
        // XXX such hacks. How to best initialize?

        /*
        RandomizeFloats(powf(0.025f, (layer / 3.0f) + 1.0), rcs[layer],
        &net->layers[layer].biases); RandomizeFloats(1.0f /
        (net->layers[layer].indices_per_node * ((layer / 3.0f) + 5)),
...
static std::tuple<int, int, int> PixelSize(const NetworkConfiguration &config,
  ...
  switch (config.style[layer]) {
    ...
    case RenderStyle::CHESSBOARD: return {256, 286, 1};
  }
}
              ...
              case RenderStyle::CHESSBOARD: {
                for (int r = 0; r < 8; r++) {
                  ...
                  for (int c = 0; c < 8; c++) {
                    ...
                    auto MostLikelyPiece = [](const vector<float> &vals,
                                              int start_idx) {
                      int maxi = 0;
                    ...
                    const int expi = expected.size() >= 64 * NUM_CONTENTS ?
                    ...
                    if (maxi != expi) {
                      string es = StringPrintf("^%d%c^2!", expi >= 7 ? 1 : 0,
                                               " PNBRQKpnbrqk"[expi]);
                ...
                for (int c = 0; c < 4; c++) {
                  const uint8 v =
                      FloatByte(stim.values[l][64 * NUM_CONTENTS + c]);
                  sdlutil::FillRectRGB(screen,
                                       xstart + c * 32 + (c >= 2 ? 130 : 0),
                ...
                const uint8 v =
                    255 - FloatByte(stim.values[l][64 * NUM_CONTENTS + 4]);
                sdlutil::FillRectRGB(screen, xstart, ystart + 32 * 8 + 8,
                                     32 * 8, 8, v, v, v);
                if (v < 127)
                  sdlutil::drawbox(screen, xstart, ystart + 32 * 8 + 8, 32 * 8,
                                   8, 0xFF, 0xFF, 0xFF);
...
static void TrainThread() {
  ...
  auto PopulateExampleFromPos = [](const Position &pos,
  ...
  double setup_ms = 0.0, stimulation_init_ms = 0.0, forward_ms = 0.0,
         ...
         output_error_ms = 0.0, update_ms = 0.0, error_history_ms = 0.0,
         eval_ms = 0.0;
  Timer total_timer;
  ...
  return start + f * height;
    ...
    if (false && CHECK_NANS /* && net->rounds == 3 */) {
      // Actually run the forward pass on CPU, trying to find the computation
      // that results in nan...
    ...
    // TODO: may make sense to pipeline this loop somehow, so that we can
    // parallelize CPU/GPU duties?
    ...
    // We do NOT propagate errors to the input layer, so dst is strictly greater
    // than 0.
    ...
    // Don't parallelize! These are all writing to the same network weights.
    // Each call is parallelized, though.
    for (int layer = 0; layer < net->num_layers; layer++) {
      ...
      // Now we leave the network on the GPU, and the version in the Network
      // object will be out of date. But flush the command queue. (why? I guess
      // make sure that we're totally done writing since other parts of the code
      // assume concurrent reads are ok?)
      uc.Finish();
      /*
      Printf("[%d/%d] = (%.2f%%) ", layer, net->num_layers, layer * 100.0 /
      net->num_layers);
      */
    }
