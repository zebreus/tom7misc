...
CL::CL() {
  ...
  if (num_platforms > 0) {
    ...
    for (int i = 0; i < num_platforms; i++) {
      ...
      for (Prop &prop : props) {
        clGetPlatformInfo(platforms[i], prop.key, 0, nullptr, &prop.size);
        prop.value = (char *)malloc(prop.size + 1);
        clGetPlatformInfo(platforms[i], prop.key, prop.size, prop.value,
                          nullptr);
      }
      ...
      if (CL_SUCCESS == clGetDeviceIDs(platforms[i], CL_DEVICE_TYPE_GPU, 0,
                                       nullptr, &platform_devices)) {
        fprintf(stderr, "      Number of GPUs: %d\n", (int)platform_devices);
        if (chosen_platform_id == -1) chosen_platform_id = i;
      } else {
