...
TPP::InputGenerator Worker::Generator(ArcFour *rc, const Goal *goal) {
  ...
  if (goal != nullptr) {
    return InputGenerator{
      ...
      p1x, p1y,
      p2x, p2y,
      sync, previous1, previous2};
  } else {
    return InputGenerator{tpp, goal, 0, 0, 0, 0, sync, previous1, previous2};
  }
}
...
vector<int> TPP::AdjacentCells(int cell) {
  ...
  auto C = [](int xx, int yy) { return yy * GRID_CELLS_W + xx; };
...
void TPP::InitTimers(const map<string, string> &config,
  ...
  if (!cache_contents.empty()) {
    ...
    for (const string &line_orig : Util::SplitToLines(cache_contents)) {
...
void TPP::InitLives(const map<string, string> &config,
  ...
  if (!cache_contents.empty()) {
    ...
    ParallelComp2D(
        ...
        // Players
        2,
        [this, &samples, &autolives1, &autolives2, &llocs1,
         &llocs2](int sample, int player) {
          const vector<uint8> &save = samples[sample];
          switch (player) {
          case 0:
          ...
          case 1:
            if (x2_loc >= 0 && x2_loc >= 0)
              llocs2[sample] = autolives2.FindLives(save, x2_loc, y2_loc, true);
            printf("2");
            break;
          }
          fflush(stdout);
...
void Worker::Visualize(const Goal *goal, vector<uint8> *argb) {
  ...
  if (goal != nullptr) {
    ...
    auto DrawGoal = [argb, gx, gy](int x1, int y1, uint8 rr, uint8 gg, uint8 bb,
                                   uint8 aa) {
  ...
  auto DrawDeaths = [argb, &mem](int loc, int xx, uint8 rr, uint8 gg, uint8 bb,
                                 uint8 aa) {
    auto DrawX = [argb, rr, gg, bb, aa](int x, int y) {
      for (int t = 0; t < XWIDTH; t++) {
        for (int w = 0; w < XTHICK; w++) {
          SetPixel(256, 256, x + t + w, y + t, rr, gg, bb, aa, argb);
          SetPixel(256, 256, x + (XWIDTH - 1 - t) + w, y + t, rr, gg, bb, aa,
                   argb);
        }
      }
    };
