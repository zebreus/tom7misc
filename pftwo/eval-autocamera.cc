  ...
  void ShowTable() {
    ...
    for (int i = 0; i < games.size(); i++) {
      ...
      printf(ANSI_WHITE "%s" ANSI_RESET ": %s" ANSI_CLEARTOEOL "\n",
             one->game.romfile.c_str(), one->message);
    }
  }
    ...
    games = ParallelMap(
        ...
        [](const Game &game) {
          ...
          for (int i = WARMUP_FRAMES; i--;) {
            emu->Step(one->movie[frameidx].first, one->movie[frameidx].second);
            frameidx++;
          }
  ...
  void DoAutoTimer() {
    ...
    ParallelComp(
        ...
        [this](int game) {
          ...
          auto MarkAddr = [one](int loc) -> string {
            if (loc == -1)
              return "-1";
...

          {
            ...
            if (outfile) {
              ...
              for (const TimerLoc &l : one->timers) {
                fprintf(outfile, "  %.2f: %d %s @%.2f = %s\n", l.score, l.loc,
                        (l.incrementing ? "++" : "--"), l.period,
  ...
  void DoAutoCamera() {
    ...
    ParallelComp3D(
        ...
        [this](int game, int sample, int player) {
          ...
          if (player == 0) {
            one->links[sample] = one->autocamera.FindLinkages(save, nullptr);
            one->xlocs1[sample] =
  ...
  [this](int game) {
    ...
    // Now compute stats.
    {
      ...
      if (outfile) {
        ...
        auto PrintPlayer = [this, &xscores1, &xscores2,
                            &MarkAddr](const vector<Linkage> &rescored) {
          for (const Linkage &l : rescored) {
            string xs1 =
                ContainsKey(xscores1, l.xloc) && xscores1[l.xloc] > 0.0f
                    ? StringPrintf(" [1p: %.2f] ", xscores1[l.xloc])
                    : "";
            string xs2 =
                ContainsKey(xscores2, l.xloc) && xscores2[l.xloc] > 0.0f
                    ? StringPrintf(" [2p: %.2f] ", xscores2[l.xloc])
                    : "";

            fprintf(outfile, "  %.2f: %d/%d = %s,%s%s%s\n", l.score, l.xloc,
                    l.yloc, MarkAddr(l.xloc).c_str(), MarkAddr(l.yloc).c_str(),
                    xs1.c_str(), xs2.c_str());
          }
        };
  ...
  void DoAutoLives() {
    ...
    ParallelComp3D(
        ...
        [this](int game, int sample, int player) {
          ...
          auto DoLives = [one, sample](const vector<Linkage> &rescored,
                                       bool player_two, vector<LivesLoc> *out) {
            if (rescored.empty())
              return;
  ...
  void ComputeStats() {
    ParallelComp(
        ...
        [this](int game) {
          ...
          auto StatsPlayer = [one](const Player &player,
                                   const vector<Linkage> &rescored,
                                   const vector<LivesLoc> &lives) {
            Stats *stats = &one->stats;
            if (player.xloc != -1 && player.yloc != -1) {
              stats->locs_known++;
            ...
            if (player.health != -1 || player.lives != -1) {
...
static void EvalAll() {
  ...
  auto Color3 = [](int found, int known, int loss) -> const char * {
    if (known == 0)
      return "";
    else if (found == known) {
      if (loss == 0)
  ...
  auto Stats3 = [](int found, int known, int loss, EvalStats *stats) {
    if (known > 0) {
      stats->has_data++;
      if (found == known)
        stats->perfect++;
  ...
  for (const Evaluation::One *g : evaluation.games) {
    ...
    printf("%s"
           ...
           Color3(s.locs_found, s.locs_known, s.locs_rank_loss), s.locs_found,
           s.locs_known, s.locs_rank_loss,

           Color3(s.lives_found, s.lives_known, s.lives_rank_loss),
           s.lives_found, s.lives_known, s.lives_rank_loss);
  }

  printf("\n"
         "  Locs: %d/%d perfect. %d any. %d total loss\n"
         " Lives: %d/%d perfect. %d any. %d total loss\n",
         locs_stats.perfect, locs_stats.has_data, locs_stats.any,
         locs_stats.total_loss, lives_stats.perfect, lives_stats.has_data,
