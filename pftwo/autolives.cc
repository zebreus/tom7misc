...
float AutoLives::IsInControl(const vector<uint8> &save,
  ...
  for (int i = 0; i < TEST_CONTROL_FRAMES; i++) {
    ...
    if (!(ram[xloc] == lram[xloc] && ram[xloc] == rram[xloc] &&
          ram[xloc] == mram[xloc]) ||
        !(ram[yloc] == lram[yloc] && ram[yloc] == rram[yloc] &&
          ram[yloc] == mram[yloc])) {
      success++;
    }
  }
    ...
    for (int f = 0; f < TRY_TO_DIE_FRAMES; f++) {
      ...
      for (auto it = locs.begin(); it != locs.end(); /* in loop */) {
        ...
        if (prev == now) {
          ...
          info->decremented++;
          info->decremented_saves.emplace_back(input16, save_before_inputs,
                                               prev, now);
...
// saves into one set.
  ...
  static_assert(MIN_COUNTER_REPETITIONS >= 2, "need 2 to find an interval!");
  if (VERBOSE)
  ...
  for (auto it = locs.begin(); it != locs.end(); /* in loop */) {
    ...
    if (info.incremented == 0) {
      ...
      auto GetConsistentDelta = [](const vector<int> &offsets,
                                   int *delta) -> int {
      ...
      for (const vector<int> &offsets : info.offsets) {
        ...
        if (reps == -1) {
          ...
          if (delta != same_delta) {
            ...
            // it somehow.
            if (VERBOSE)
              printf(" %d != %d <exit>", delta, same_delta);
            same_delta = -2;
            break;
          }
        } else {
  ...
  for (auto it = locs.begin(); it != locs.end(); ++it) {
    ...
    auto AnalyzeFrames = [&/* XXX */]() {
      for (const Frame &frame : info.decremented_saves) {
        ...
        const float base_control =
            IsInControl(emu->SaveUncompressed(), xloc, yloc, player_two);
        ...
        if (dbase != dalt) {
          if (VERBOSE)
            printf("%04x failed alt test: base %d->%d alt %d->%d\n", loc,
                   base_value_before, base_value_after, alt_value_before,
                   alt_value_after);
          return false;
        }
        const float alt_control =
            IsInControl(emu->SaveUncompressed(), xloc, yloc, player_two);
        ...
        auto Test = [this, emu, xloc, yloc, player_two, loc, &frame,
                     base_control, base_value_before,
                     base_value_after](uint8 set_to, int *tests, float *score) {
          emu->LoadUncompressed(frame.save);
          ...
          if (expt_value_before > set_to) {
            ...
            const float expt_control =
                IsInControl(emu->SaveUncompressed(), xloc, yloc, player_two);
            if (VERBOSE)
              printf("%02x) ctl %.3f", expt_value_after, expt_control);
