...
struct UIThread {
  ...
  void Loop() {
    ...
    for (;;) {
...

      {
        ...
        for (int i = 0; i < workers.size(); i++) {
          ...
          case STATUS_EXPLORE: {
            ...
            sdlutil::drawbox(screen, xx, yy, bar_on, BAR_HEIGHT, rr, gg, bb);
            sdlutil::drawbox(screen, xx + bar_on, yy, BAR_WIDTH - bar_on,
                             BAR_HEIGHT, rr >> 1, gg >> 1, bb >> 1);
            break;
          }
          case STATUS_DIE:
            sdlutil::drawbox(screen, xx, yy, BAR_WIDTH, BAR_HEIGHT, 0xA0, 0x00,
                             0x00);
            break;
          case STATUS_UNKNOWN:
            sdlutil::drawbox(screen, xx, yy, BAR_WIDTH, BAR_HEIGHT, 0x60, 0x60,
                             0x00);
            break;
          case STATUS_TREE:
            sdlutil::drawbox(screen, xx, yy, BAR_WIDTH, BAR_HEIGHT, 0xA0, 0xA0,
                             0xA0);
            break;
          }
...

      {
        ...
        for (int cy = 0; cy < Problem::GRID_CELLS_H; cy++) {
          for (int cx = 0; cx < Problem::GRID_CELLS_W; cx++) {
            ...
            if (gc.node == nullptr) {
              sdlutil::FillRectRGB(screen, GRIDX + cx * CELLPX,
                                   GRIDY + cy * CELLPX, CELLPX, CELLPX, 0x20,
                                   0x0, 0x0);
            } else {
              uint8 rr = gc.score >= cutoff_bestscore ? 0 : 1;
              int gg = 0x20 + (0xFF - 0x20) * (gc.score * one_over_bestscore);
              if (gg > 0xFF)
                gg = 0xff;
              sdlutil::FillRectRGB(screen, GRIDX + cx * CELLPX,
                                   GRIDY + cy * CELLPX, CELLPX, CELLPX, rr * gg,
                                   (uint8)gg, 0x0);
            }
          }
        }
        ...
        // for (int cy = 0; cy < 256; cy++) {
        {
          ...
          for (int cx = 0; cx < 2048 && cx < WIDTH; cx++) {
            ...
            if (gc.node == nullptr) {
              sdlutil::drawpixel(screen, MEMX + cx, MEMY + cy, 0x20, 0x00,
                                 0x00);
            } else {
              uint8 rr = gc.score >= cutoff_bestscore ? 0 : 1;
              int gg = 0x20 + (0xFF - 0x20) * (gc.score * one_over_bestscore);
              if (gg > 0xFF)
                gg = 0xff;
              sdlutil::drawpixel(screen, MEMX + cx, MEMY + cy, rr * gg,
                                 (uint8)gg, 0x00);
            }
          }
        }
      ...
      for (int i = 0; i < texts.size() && i < max_screenshots; i++) {
        int x = i % 11;
        int y = i / 11;
        BlitARGBHalf(screenshots[i], 256, 256, x * 128, y * 128 + 20, screen);

        if (!queue_mode && texts.size() < 12) {
          for (int t = 0; t < texts[i].size(); t++) {
            font->draw(x * 128, (y + 1) * 128 + 20 + t * FONTHEIGHT,
                       texts[i][t]);
          }
        }
      }
      ...
      std::function<void(int, const Tree::Node *)> GetLevelStats =
          [this, &levels, &treestats, &all_scores,
           &GetLevelStats](int depth, const Tree::Node *n) {
      ...
      for (int x = 0; x < all_scores.size() && x < WIDTH; x++) {
        ...
        for (int h = 0; h < height; h++) {
          int y = BOTTOM - h;
          sdlutil::drawpixel(screen, x, y, 0xFF, 0xFF, over ? 0xFF : 0x00);
        }
        sdlutil::drawpixel(screen, x, BOTTOM - height, leftover, leftover,
                           0x00);
      }
  ...
  void Run() {
    ...
    smallfont = Font::Create(screen, "fontsmall.png", FONTCHARS, SMALLFONTWIDTH,
                             SMALLFONTHEIGHT, FONTSTYLES, 0, 3);
    CHECK(smallfont != nullptr) << "Couldn't load smallfont.";

    hugefont = Font::Create(screen, "fonthuge.png", FONTCHARS, HUGEFONTWIDTH,
                            HUGEFONTHEIGHT, FONTSTYLES, 4, 3);
