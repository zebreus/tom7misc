...
#ifndef _PFTWO_WEIGHTED_OBJECTIVES_H
#define _PFTWO_WEIGHTED_OBJECTIVES_H
...
struct WeightedObjectives {
  // Use flat weights.
  explicit WeightedObjectives(const std::vector<std::vector<int>> &objs);
  // Use provided weights.
  explicit WeightedObjectives(
      std::vector<std::pair<std::vector<int>, double>> w_objs);
  // Weight using the example memories.
  WeightedObjectives(const std::vector<std::vector<int>> &objs,
  ...
  double WeightedLess(const std::vector<uint8_t> &mem1,
                      const std::vector<uint8_t> &mem2) const;

  // Scoring function which is the count of objectives
  // where mem1 < mem2 minus the number where mem1 > mem2.
  double Evaluate(const std::vector<uint8_t> &mem1,
                  const std::vector<uint8_t> &mem2) const;

  // XXX weighted version, unnormalized version?
  const std::vector<std::pair<std::vector<int>, double>> &GetAll() const {
    return weighted;
  }

  const std::pair<std::vector<int>, double> &Get(int i) const {
    return weighted[i];
  }
 ...
 private:
  std::vector<std::pair<std::vector<int>, double>> weighted;
...
struct Observations {
  ...
  virtual void Accumulate(const std::vector<uint8_t> &memory) = 0;
  ...
  virtual double GetNormalizedValue(const std::vector<uint8_t> &memory) = 0;

  // As GetNormalizedValue, but the weighted average of each fraction.
  // In [0, 1].
  virtual double GetWeightedValue(const std::vector<uint8_t> &memory) = 0;
  ...
  virtual void VizText(const std::vector<uint8_t> &mem,
