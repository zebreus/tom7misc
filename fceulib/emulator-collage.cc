// This is a fork of the ambitious emulator_test.cc, which
// I replaced with simpler standalone tests in 2022.
// It doesn't work, but probably could be made to work again,
// and it would be good to have a comprehensive test suite!

// The checksums in here are certainly wrong, because I switched
// this to use the new combined CPU/RAM checksum (MachineChecksum),
// but didn't update anything because it already wasn't working.
...
# define CHECK_RAM_STEP(i) do {                                 \
  ...
  // XXX should internally do machine state checksums too.
  ...
  const uint64 ret1 = emu->MachineChecksum();
  ...
  const uint64 ret2 = emu->MachineChecksum();
