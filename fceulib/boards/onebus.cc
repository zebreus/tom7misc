...
struct UNLOneBus final : public CartInterface {
  ...
  // IRQ Registers
  uint8 IRQCount = 0, IRQa = 0, IRQReload = 0;
  #define IRQLatch cpu410x[0x1]

  // MMC3 Registers
  // some OneBus Systems have swapped PRG reg commands in MMC3 implementation,
  ...
  // APU Registers
  uint8 pcm_enable = 0, pcm_irq = 0;
  int16 pcm_addr = 0, pcm_size = 0, pcm_latch = 0, pcm_clock = 0xF6;

  writefunc defapuwrite[64];
  readfunc defapuread[64];
  ...
  void PSync() {
    ...
    fc->cart->setprg8(0x8000 ^ pswap, block | (bank0 & mask));
    //  FCEU_printf(" %04x [%02x]",0xa000^pswap,block | (bank1 & mask));
    fc->cart->setprg8(0xa000, block | (bank1 & mask));
    //  FCEU_printf(" %04x [%02x]",0xc000^pswap,block | (bank2 & mask));
    fc->cart->setprg8(0xc000 ^ pswap, block | (bank2 & mask));
    //  FCEU_printf(" %04x [%02x]\n",0xe000^pswap,block | (bank3 & mask));
    fc->cart->setprg8(0xe000, block | (bank3 & mask));
  }
  ...
  void CSync() {
    ...
    fc->cart->setchr1(0x0000 ^ cswap, block | (bank0 & mask));
    fc->cart->setchr1(0x0400 ^ cswap, block | (bank1 & mask));
    fc->cart->setchr1(0x0800 ^ cswap, block | (bank2 & mask));
    fc->cart->setchr1(0x0c00 ^ cswap, block | (bank3 & mask));
    fc->cart->setchr1(0x1000 ^ cswap, block | (bank4 & mask));
    fc->cart->setchr1(0x1400 ^ cswap, block | (bank5 & mask));
    fc->cart->setchr1(0x1800 ^ cswap, block | (bank6 & mask));
    fc->cart->setchr1(0x1c00 ^ cswap, block | (bank7 & mask));

    fc->cart->setmirror((mirror & 1) ^ 1);
  }
  ...
  void UNLOneBusWriteCPU410X(DECLFW_ARGS) {
  ...
  void UNLOneBusWritePPU201X(DECLFW_ARGS) {
    //  FCEU_printf("PPU %04x:%04x\n",A,V);
    ppu201x[A & 0x0f] = V;
    Sync();
  }

  void UNLOneBusWriteMMC3(DECLFW_ARGS) {
  ...
  void UNLOneBusWriteAPU40XX(DECLFW_ARGS) {
  ...
  DECLFR_RET UNLOneBusReadAPU40XX(DECLFR_ARGS) {
  ...
  static void StateRestore(FC *fc, int version) {
    ((UNLOneBus *)fc->fceu->cartiface)->Sync();
  }

  UNLOneBus(FC *fc, CartInfo *info) : CartInterface(fc) {
    ...
    fc->ppu->GameHBIRQHook = [](FC *fc) {
      ((UNLOneBus *)fc->fceu->cartiface)->UNLOneBusIRQHook();
    };
    fc->X->MapIRQHook = [](FC *fc, int a) {
      return ((UNLOneBus *)fc->fceu->cartiface)->UNLOneBusCpuHook(a);
    };
    fc->fceu->GameStateRestore = StateRestore;
    fc->state->AddExVec({{cpu410x, 16, "REGC"},
                         {ppu201x, 16, "REGS"},
                         {apu40xx, 64, "REGA"},
                         {&IRQReload, 1, "IRQR"},
                         {&IRQCount, 1, "IRQC"},
