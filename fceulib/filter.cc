...
void Filter::SexyFilter(int32 *in, int32 *out, int32 count) {
  int32 mul1 = (94 << 16) / FCEUS_SNDRATE;
  int32 mul2 = (24 << 16) / FCEUS_SNDRATE;
  int32 vmul = (FCEUS_SOUNDVOLUME << 16) * 3 / 4 / 100;
...
int32 Filter::NeoFilterSound(int32 *in, int32 *out, uint32 inlen,
                             int32 *leftover) {
  ...
  if (FCEUS_SOUNDQ == 2) {
    for (x = mrindex; x < max; x += mrratio) {
      ...
      acc = ((int64)acc * (65536 - (x & 65535)) + (int64)acc2 * (x & 65535)) >>
            (16 + 11);
      *out = acc;
      out++;
      count++;
  ...
  if (fc->sound->GameExpSound.NeoFill)
    fc->sound->GameExpSound.NeoFill(fc, outsave, count);

  SexyFilter(outsave, outsave, count);
  if (FCEUS_LOWPASS) SexyFilter2(outsave, count);
  return count;
}

void Filter::MakeFilters(int32 rate) {
  const int32 *tabs[6] = {C44100NTSC, C44100PAL,  C48000NTSC,
                          C48000PAL,  C96000NTSC, C96000PAL};
  const int32 *sq2tabs[6] = {SQ2C44100NTSC, SQ2C44100PAL,  SQ2C48000NTSC,
                             SQ2C48000PAL,  SQ2C96000NTSC, SQ2C96000PAL};

  const uint32 nco = FCEUS_SOUNDQ == 2 ? SQ2NCOEFFS : NCOEFFS;

  mrindex = (nco + 1) << 16;
  mrratio =
      (fc->fceu->PAL ? (int64)(PAL_CPU * 65536) : (int64)(NTSC_CPU * 65536)) /
      rate;

  const int idx = (fc->fceu->PAL ? 1 : 0) | (rate == 48000 ? 2 : 0) |
                  (rate == 96000 ? 4 : 0);
